...
- **セットアップ画面** - サーバーURLデフォルトプリセット + 未割当クライアント一覧から選択して接続完了 (Client Key手入力フォールバックあり)、接続テスト、設定変更・初期化対応
...
| 決定 / ENTER | 一時停止/再開 |
| 上 / 下 | 無効 (WebView操作を防止) |
| KEYCODE 93 | デバッグオーバーレイ切替 (ログ→スケジュール→端末情報→命名マニュアル→消去) |
| F1-F4, VOL+/- | デバッグログに記録 |
...
```
アプリ起動
  ├── config なし → セットアップ画面 (URLデフォルトプリセット)
  │     ├── 未割当クライアントを取得 → 一覧から選択 → 自動接続テスト → 保存して再生開始
  │     └── フォールバック: Client Key手入力 → 接続テスト → 保存
...
再生中の設定変更
  5秒長押し → 設定画面 (現在の値が入った状態)
    ...
    ├── 戻る → 変更せずPlayerに戻る
    └── 設定を初期化 → 確認ダイアログ → OK → URLはデフォルト値・Keyは空に
```
...
SMB共有フォルダ内のPDF/URLテキストファイル名で表示制御が可能。規約外のファイル名はデフォルト動作。

### 書式

```
{順番}_{ページ制御}_{表示開始日}_{表示終了日}[_{表示秒数}]_{説明}.pdf
{順番}_{ページ制御}_{表示開始日}_{表示終了日}[_{表示秒数}]_{説明}.txt
```
...
|-----------|------|-----|
| 順番 | 表示順 (数値・小さい順) | 001 |
| ページ制御 | 0=先頭ページのみ / 1=全ページ表示 (txtの場合は無視) | 0 |
| 表示開始日 | yyyyMMdd形式 | 20260501 |
| 表示終了日 | yyyyMMdd形式 | 20260531 |
| 表示秒数 | 秒数 (省略可。省略時は親アイテムのデフォルト秒数。全ページ時は1ページあたり、txt複数URL時は1URLあたりの秒数) | 30 |
| 説明 | 任意テキスト | 書簡 |

### .txt ファイル (URL表示)

- 中身にURL (`http://` または `https://` で始まる行) を記載するとWeb画面として表示
- 複数行にURLを書くと1URLずつ指定秒数でローテーション表示
- URL以外の行は無視される

### 例

```
001_0_20260501_20260531_30_書簡.pdf         → 先頭ページのみ、30秒、5/1〜5/31表示
002_1_20260601_20261231_10_全頁資料.pdf     → 全ページ、10秒/ページ、6/1〜12/31表示
003_0_20250101_20250131_30_期限切れ.pdf     → 日付範囲外で非表示
004_0_20260501_20260630_AITOOL.txt          → 中身のURLをWeb表示 (秒数省略=デフォルト秒数)
005_0_20260501_20260630_10_AIニュース.txt   → 複数URLを10秒ごとに1URLずつ表示
既存ファイル.pdf                              → 規約外:デフォルト動作、規約ファイルの後に配置
```
