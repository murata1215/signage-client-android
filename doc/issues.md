...
- [~] **実装完了・ビルド成功（2026-09-04）。実機検証待ち**
  - リモコンの音量/消音がシステム側で制御できることを確認済みのため、アプリ側は音量/ミュートUIを持たず
    「ミュートで再生開始→PLAYING実測後にアンミュート」のみ実装。詳細は`doc/changelog.md`参照
  - リリース: versionCode 91 / versionName "1.91"
  - 次アクション: `dt-astbx5-01`で実機検証(音が出る/paused after unmuteが出ない/画面送りで即無音/
    先読み中に裏で鳴らない/web・PDF・SMBのデグレ無し) → 問題なければ`本番反映.bat`で本番4台へ

## `dt-astbx5-04` YouTubeエラー5

- [x] **解決済み（2026-09-04）。原因: WebView版数差、ファームウェア更新で解消（コード変更なし）**
  - 症状: `-04`のみYouTubeがタイトル表示直後にエラーでスキップされる
  - 解像度差(仮説A)・ネットワーク差、いずれも実測で棄却。WebView版数(`-01`=146.0.7680.119 /
    `-04`=140.0.7298.0、共にAOSP実装のためPlayストア更新不可)が原因と特定
  - I-O DATA公式手順でファームウェア更新 → YouTube再生成功。詳細は`doc/changelog.md`参照
  - 残タスク: `-02`/`-03`のWebView版数確認、140系ならファーム更新を推奨(予防)

## YouTubeライブ配信で次画面へ進まない + ステータスバー固着 (v1.90)

- [x] **実装完了・実機検証OK（2026-09-04、ファーム更新後の`-04`含め正常動作を確認）**
