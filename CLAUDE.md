...
- **プロキシ**: コンテンツ単位(管理画面の「プロキシを使用」＋「プロキシURL」)で指定する。端末側にプロキシ設定・ドメインバイパスは存在しない。サーバー通信は常に direct 固定(v1.92)
...
- **社内プロキシ端末固定設定の廃止 + コンテンツ単位プロキシ移行(v1.92)**: signage-serverの客先環境展開に備え、端末に社内プロキシ(`210.175.128.100:8080`)と22件のドメインバイパスリストをハードコードしていた従来方式を廃止し、管理画面で設定されたコンテンツ単位の`use_proxy`/`proxy_url`(サーバー側で実装済み・`PlaylistItem`には既に受信済みだが未使用だった)を実際に解釈して適用する方式に移行した。新設`data/ProxySpec.kt`が`proxy_url`文字列(`"http://host:port"`/`"host:port"`/スキームありポート省略時はデフォルト80等)をパースし、WebView用`toRule()`("host:port")とOkHttp用`toJavaProxy()`(`java.net.Proxy`)の両方を提供する(`parse()`は例外を投げず不正入力は全てnull=direct扱い、JUnitテスト`ProxySpecTest`で受理仕様表を網羅)。`ServerClient.kt`はハードコードproxyHost/proxyPort/bypassPrefixes(22件)/`getClient()`分岐を全廃し、サーバー通信(schedule/heartbeat/PDF DL/update等)は常に単一`directClient`固定にした(客先環境でも社内プロキシ経由にならないよう明示的に分離)。`probeYoutubeConnectivity(proxy: ProxySpec? = null)`は呼び出し側がプレイリスト中の`use_proxy=true`項目の`proxy_url`を渡した場合だけその一時OkHttpClientで疎通確認する設計に変更(渡されなければdirect)。新設`player/WebProxyManager.kt`(singleton)が`androidx.webkit.ProxyController`の`setProxyOverride`/`clearProxyOverride`(プロセス全体に効くAPI)を唯一の窓口として直列管理し、`apply(target, onReady)`は対象が現在適用中の状態と同じ(null同士も等価)かつ初期適用済みならAPIを呼ばず即`onReady`、異なれば明示的に切替(target==nullでも必ず`clearProxyOverride`を呼び前回設定の残留を防止)、非対応端末は常にdirect扱いでフォールバック。`PlayerActivity.kt`はハードコードproxy/バイパスルールを設定していた`setupWebViewProxy()`(約46行)を削除し、全WebViewロード箇所を`WebProxyManager.apply(specFor(screen)) { loadUrl/loadScreen(...) }`経由に統一(`specFor(screen) = if (item.useProxy) ProxySpec.parse(item.proxyUrl) else null`)。3枚WebView(active+next+prev)クロスフェード先読みアーキテクチャとの整合のため、次/前画面のプロキシ指定が現在適用中の状態と異なる場合は背景プリロードをスキップ(`preloadScreen()`冒頭でガード、表示中コンテンツの通信がプロセス全体のプロキシ切替で巻き添えを食うのを防止)し、実際の切替タイミングで`doAdvance()`/`goToPrevious()`が同期的に`WebProxyManager.apply()→loadScreen()→再突入`する。あわせて、ローテーション再利用(WebViewを再ロードせずactive→next/prevへ転用してready扱いにする既存の最適化)によるプロキシ状態の陳腐化を防ぐため`nextReadySpec`/`prevReadySpec`を新設し、preload完了時・ローテーション再利用時の両方で「そのWebViewが実際にロードされた時点のspec」を記録、ready判定は`readyフラグ && readySpec==targetSpec`の両方を満たす場合のみ信頼する(片方でも不一致なら同期切替パスにフォールバック)設計を追加した(プロキシ切替直後に折り返しナビゲーションした場合の白画面事故を防ぐための、計画原文を上回る予防的拡張)。デバッグオーバーレイ(3ページ目)のハードコード`"プロキシ: 210.175.128.100:8080"`表示は、実際の`WebProxyManager.current`("なし（direct）"/`host:port`)・現在コンテンツのuse_proxy/proxy_url・パース失敗時の`⚠ プロキシURL不正`警告・`サーバー通信: direct（固定）`の動的表示に置き換えた。`data/ConfigManager.kt`/`ui/SetupScreen.kt`/`data/Models.kt`は無改修(端末単位のプロキシ設定UIは追加せず、`PlaylistItem`の`useProxy`/`proxyUrl`は既存フィールドをそのまま再利用したため)
